/**
 * Generate grammar notes for "gyofukuki" (3 difficulties) and merge
...
import { gyofukukiSimplified, gyofukukiIntermediate, gyofukukiOriginal } from '../src/data/books/gyofukuki';
...
const versions = {
  simplified: gyofukukiSimplified,
  intermediate: gyofukukiIntermediate,
  original: gyofukukiOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for gyofukuki/${diff} (${versions[diff].length} chars)...`);
