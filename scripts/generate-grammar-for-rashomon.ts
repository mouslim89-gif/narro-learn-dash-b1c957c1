/**
 * Generate grammar notes for "rashomon" (3 difficulties) and merge
 * into book-grammar.ts. Idempotent — re-run to refresh.
 *
 * Run: npx tsx scripts/generate-grammar-for-rashomon.ts
 */
import * as fs from 'fs';
import * as path from 'path';
import { rashomonSimplified, rashomonIntermediate, rashomonOriginal } from '../src/data/books/rashomon';
...
const versions = {
  simplified: rashomonSimplified,
  intermediate: rashomonIntermediate,
  original: rashomonOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for rashomon/${diff} (${versions[diff].length} chars)...`);
