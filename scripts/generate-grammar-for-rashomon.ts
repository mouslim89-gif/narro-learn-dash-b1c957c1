/**
 * Generate grammar notes for "rashomon" — one call per (difficulty, part).
...
const partsByDiff = {
  simplified: rashomonSimplifiedParts,
  intermediate: rashomonIntermediateParts,
  original: rashomonOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for rashomon/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
