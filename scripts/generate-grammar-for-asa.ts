/**
 * Generate grammar notes for "asa" — one call per (difficulty, part).
...
const partsByDiff = {
  simplified: asaSimplifiedParts,
  intermediate: asaIntermediateParts,
  original: asaOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for asa/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
