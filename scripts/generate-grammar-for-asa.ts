/**
 * Generate grammar notes for "asa" (3 difficulties) and merge
 * into book-grammar.ts. Idempotent — re-run to refresh.
 *
 * Run: npx tsx scripts/generate-grammar-for-asa.ts
 */
import * as fs from 'fs';
import * as path from 'path';
import { asaSimplified, asaIntermediate, asaOriginal } from '../src/data/books/asa';
...
const versions = {
  simplified: asaSimplified,
  intermediate: asaIntermediate,
  original: asaOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for asa/${diff} (${versions[diff].length} chars)...`);
