/**
 * Generate grammar notes for "matsu" (3 difficulties) and merge
...
import { matsuSimplified, matsuIntermediate, matsuOriginal } from '../src/data/books/matsu';
...
const versions = {
  simplified: matsuSimplified,
  intermediate: matsuIntermediate,
  original: matsuOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for matsu/${diff} (${versions[diff].length} chars)...`);
