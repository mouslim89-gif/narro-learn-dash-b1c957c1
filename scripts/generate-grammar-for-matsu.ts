/**
 * Generate grammar notes for "matsu" — one call per (difficulty, part).
 * Writes back into book-grammar.ts under the GrammarNote[][] shape.
 *
 * Run: npx tsx scripts/generate-grammar-for-matsu.ts
 */
import * as fs from 'fs';
import * as path from 'path';
import {
  matsuSimplifiedParts,
  matsuIntermediateParts,
  matsuOriginalParts,
} from '../src/data/books/matsu';
...
const partsByDiff = {
  simplified: matsuSimplifiedParts,
  intermediate: matsuIntermediateParts,
  original: matsuOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for matsu/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
