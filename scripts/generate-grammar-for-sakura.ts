/**
 * Generate grammar notes for "sakura" — one call per (difficulty, part).
 * Writes back into book-grammar.ts under the GrammarNote[][] shape.
 *
 * Run: npx tsx scripts/generate-grammar-for-sakura.ts
 */
import * as fs from 'fs';
import * as path from 'path';
import {
  sakuraSimplifiedParts,
  sakuraIntermediateParts,
  sakuraOriginalParts,
} from '../src/data/books/sakura';
...
const partsByDiff = {
  simplified: sakuraSimplifiedParts,
  intermediate: sakuraIntermediateParts,
  original: sakuraOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for sakura/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
