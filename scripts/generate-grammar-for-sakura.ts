/**
 * Generate grammar notes for "sakura" (3 difficulties) and merge
...
import { sakuraSimplified, sakuraIntermediate, sakuraOriginal } from '../src/data/books/sakura';
...
const versions = {
  simplified: sakuraSimplified,
  intermediate: sakuraIntermediate,
  original: sakuraOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for sakura/${diff} (${versions[diff].length} chars)...`);
