/**
 * Generate grammar notes for "kumo-no-ito" — one call per (difficulty, part).
 * Writes back into book-grammar.ts under the GrammarNote[][] shape.
 *
 * Run: npx tsx scripts/generate-grammar-for-kumo.ts
 */
...
import {
  ...
  kumoOriginalParts,
} from '../src/data/books/kumo-no-ito';
...
const partsByDiff = {
  simplified: kumoSimplifiedParts,
  intermediate: kumoIntermediateParts,
  original: kumoOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for kumo-no-ito/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
