/**
 * Generate grammar notes for "kumo-no-ito" (3 difficulties) and merge
...
import { kumoSimplified, kumoIntermediate, kumoOriginal } from '../src/data/books/kumo-no-ito';
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for kumo-no-ito/${diff} (${versions[diff].length} chars)...`);
