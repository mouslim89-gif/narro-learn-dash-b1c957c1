/**
 * Generate grammar notes for "urashima" — one call per (difficulty, part).
...
const partsByDiff = {
  simplified: urashimaSimplifiedParts,
  intermediate: urashimaIntermediateParts,
  original: urashimaOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for urashima/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
