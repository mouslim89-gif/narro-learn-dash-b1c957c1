/**
 * Generate grammar notes for "urashima" (3 difficulties) and merge
 * into book-grammar.ts. Idempotent — re-run to refresh.
 *
 * Run: npx tsx scripts/generate-grammar-for-urashima.ts
 */
import * as fs from 'fs';
import * as path from 'path';
import { urashimaSimplified, urashimaIntermediate, urashimaOriginal } from '../src/data/books/urashima';
...
const versions = {
  simplified: urashimaSimplified,
  intermediate: urashimaIntermediate,
  original: urashimaOriginal,
};
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    console.log(`Fetching grammar for urashima/${diff} (${versions[diff].length} chars)...`);
