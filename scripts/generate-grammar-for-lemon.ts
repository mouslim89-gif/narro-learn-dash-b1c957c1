/**
 * Generate grammar notes for "lemon" — one call per (difficulty, part).
...
const partsByDiff = {
  simplified: lemonSimplifiedParts,
  intermediate: lemonIntermediateParts,
  original: lemonOriginalParts,
} as const;
...
async function main() {
  ...
  for (const diff of ['simplified', 'intermediate', 'original'] as const) {
    ...
    for (let i = 0; i < parts.length; i++) {
      console.log(`Fetching grammar for lemon/${diff} part ${i + 1}/${parts.length} (${parts[i].length} chars)...`);
