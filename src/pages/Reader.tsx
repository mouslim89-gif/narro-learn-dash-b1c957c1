...
export default function Reader() {
  ...
  // Defensive: never render any parenthesised content in the reader.
  // Source texts must already be cleaned of furigana/glosses; this is a safety net.
  ...
  const tokens = useMemo(() => {
    ...
    if (raw && raw.length > 0) {
      const cleaned = raw.filter((tk) => !/^[（(].*[）)]$/.test(tk.t));
