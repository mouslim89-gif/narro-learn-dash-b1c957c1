...
export default function Reader() {
  ...
  // Strip inline furigana written in parentheses (kana-only contents).
  // Keeps explanatory glosses that include kanji, e.g. かつら（頭にかぶる髪）.
  const stripParenFurigana = (text: string): string =>
    text.replace(/[（(]\s*([\u3040-\u309F\u30A0-\u30FFー・\s　]+)\s*[）)]/g, '');
  ...
  const tokens = useMemo(() => {
    ...
    if (raw && raw.length > 0) {
      // Drop any leftover parenthesised furigana tokens defensively.
      const cleaned = raw.filter(
        (tk) => !/^[（(]\s*[\u3040-\u309F\u30A0-\u30FFー・\s　]+\s*[）)]$/.test(tk.t),
      );
      return mergeConjugatedTokens(cleaned);
    }
    // Fallback — split text into char-level tokens
    const fallbackText = book ? stripParenFurigana(getChapterContent(book, chapterId, difficulty)) : '';
    const out: BookToken[] = [];
  ...
  const bookText = useMemo(() => {
    if (!book) return '';
    return stripParenFurigana(getChapterContent(book, chapterId, difficulty));
